...
@section('content')
    ...
    <div class="content-wrapper">
        <div class="media-section">
            ...
            {{-- <div class="home__img-container">
                @php
                    $mediaFiles = is_array($room->rooms_media) ? $room->rooms_media : explode(',', $room->rooms_media);
                @endphp
                @if (!empty($mediaFiles) && $mediaFiles[0] != '')
                    <img class="home__img" src="{{ asset($mediaFiles[0]) }}" alt="{{ $room->rooms_name }}" />
                @else
                ...
                <article class="description__content">
                    <div class="description__header" style="display: flex; flex-direction: column;">
                        ...
                        <div class="class-section">
                            ...
                            <span class="class-data">{{ $room->roomClass->classes_name }}</span>
                            <br>
                            <span class="class-label">Tersedia: </span>
                            <span class="class-data">{{ $room->jumlah_kamar }} Kamar</span>
                        </div>
                    </div>

                    <p class="description__text" style="text-align: justify;">
                    <div>{{ strip_tags($room->description) }}</div>
                    </p>
                    <div class="description__footer">
                        <div class="facilities-rules-container">
                            <div class="facilities-section">
                            ...
                            <div class="rules-section">
                                <h5>Fasilitas Kamar:</h5>
                                <ul>
                                    @foreach (json_decode($room->roomClass->facilities) as $facilitiesRoom)
                                        <li>{{ $facilitiesRoom }}</li>
                                    @endforeach
                                </ul>
                            </div>
                        ...
                        <article class="property__card">
                            <a href="{{ route('rooms.show', $otherRoom->slug) }}" class="other-room-link"
                                ...
                                <div class="property__images">
                                    ...
                                    @if (!empty($otherRoomMediaFiles) && $otherRoomMediaFiles[0] != '')
                                        <img src="{{ asset($otherRoomMediaFiles[0]) }}"
                                            alt="{{ $otherRoom->rooms_name }}" class="property__img" />
                                    @else
                                ...
                                <div class="property__data">
                                    <h3 class="property__title">{{ $otherRoom->rooms_name }}</h3>
                                    <span class="property__description">{{ $otherRoom->roomClass->price }}</span>
                                    <span class="property__description">{{ $otherRoom->roomClass->classes_name }}</span>
                                </div>
                            </a>
                        </article>
                    ...
                    @foreach ($otherAvailableRooms->where('status', 'Tersedia') as $otherRoom)
                        <article class="property__card">
                            <a href="{{ route('rooms.show', $otherRoom->slug) }}" class="other-room-link"
                                ...
                                <div class="property__images">
                                    @php
                                        $mediaFiles = explode(',', $otherRoom->rooms_media);
                                    @endphp

                                    @if (!empty($mediaFiles) && $mediaFiles[0] != '')
                                        <img src="{{ asset($mediaFiles[0]) }}" alt="{{ $otherRoom->rooms_name }}"
                                            class="property__img" />
                                    @else
                                        <img src="{{ asset('placeholder.jpg') }}" alt="No Room Image"
                                            class="property__img" />
    ...
    {{-- @section('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.querySelectorAll('.other-room-link').forEach(function(link) {
                    link.addEventListener('click', function(event) {
                        ...
                        fetch('/rooms/' + roomSlug)
                            .then(response => response.json())
                            .then(data => {
                                document.querySelector('.home__title').textContent = data.kost
                                    .name_kost;
                                document.querySelector('.home__container h2').textContent = data
                                    .room.rooms_name;
                                document.querySelector('.home__img').src = data.mediaFiles[0];
                                document.querySelector('.home__img').alt = data.room.rooms_name;
                                ...
                                document.querySelector('.description__text div').textContent = data
                                    .kost.description;
                                document.querySelector('.location-container span').textContent =
                                    data.kost.location;

                                var facilitiesSection = document.querySelector(
                                ...
                                updateSocialMediaLink('.social-media-link.bxl-whatsapp', data.kost
                                    .whatsapp_number, 'https://wa.me/');
                                updateSocialMediaLink('.social-media-link.bxl-facebook', data.kost
                                    .facebook);
                                updateSocialMediaLink('.social-media-link.bxl-instagram', data.kost
                                    .instagram);
                                updateSocialMediaLink('.social-media-link.bxl-twitter', data.kost
                                    .twitter);

                                // Mengaktifkan kembali fungsionalitas galeri
                                const largeImage = document.querySelector(".home__img");
                                const images = document.querySelectorAll(".galleries img");
                                images.forEach((image, i) => {
                                    image.addEventListener("click", () => {
                ...
                // Ganti gambar utama saat gambar galeri diklik
                const largeImage = document.querySelector(".home__img");
                const images = document.querySelectorAll(".galleries img");
                images.forEach((image, i) => {
                    image.addEventListener("click", () => {
